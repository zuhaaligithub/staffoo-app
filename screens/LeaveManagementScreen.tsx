...
export default function LeaveManagementScreen({ navigation }: Props) {
  ...
  const [selectedGuardId, setSelectedGuardId] = useState<number | undefined>(
    undefined,
  );
  ...
  const approveLeave = async (leaveId: number) => {
    Alert.alert("Approve Leave", "Are you sure?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await postWithAuth(`${BASE_URL}/approveLeave`, {
              id: leaveId,
            });
  ...
  const submitLeave = async () => {
    ...
    try {
     await postWithAuth(`${BASE_URL}/addAdminLeaveRequest`, payload);
          ...
          filteredLeaves.map((item) => (
            <View key={item.id} style={styles.card}>
              <View style={styles.infoRow}>
                <Text style={styles.name}>{item.guardss?.name || "Staff"}</Text>
                <Text style={styles.value}>{item.guardss?.email}</Text>
              </View>
              <View style={styles.infoRow}>
              ...
              <View style={styles.footer}>
                ...
                {isContractor && item.status?.toLowerCase() === "approved" && (
                  <TouchableOpacity
                    ...
                    onPress={() => actionOnApproved(item.id)}
                  >
