...
//       const profileRes = await axios.get(`${BASE_URL}/user-edit/${userId}`, {
//         headers: {
...
//         `${BASE_URL}/get-contractor-rates/${userId}`,
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
//             Accept: "application/json",
...
//   // Sort states alphabetically by label so the list of state cards is stable/predictable
...
//   // Whether a given state code already has an approved/existing rate entry
...
//   // Are there any allowed states still missing rates? (drives header button state)
...
//     // If a specific state was clicked → only that state
//     // Otherwise (header button) → all allowed states
//     const statesToShow = preferredState
//       ? [preferredState.toLowerCase()].filter((c) => codes.includes(c))
//       : codes;
...
//     statesToShow.forEach((code) => {
//       // Try to pre-fill from existing rates
...
//     setSelectedStates(statesToShow);
//     setActiveStateTab(statesToShow[0]);
//     setAdminNotes("");
...
//         throw new Error("No state selected for rate request");
//       }
...
//                 profile. Please request charge rates for states you selected in
//                 profile section.
//               </Text>
//             </View>
...
//           }
//         >
//           {/* Rates Tables — one card per state that has rates (no tabs, state managed from header button) */}
//           {sortedRatesList.map((rate) => {
//             const stateCode = String(rate.state || "").toLowerCase();
//             const rows = buildRowsForRate(rate);
//             return (
//               <View key={String(rate.id ?? stateCode)} style={styles.tableCard}>
...
//                       const active = code === activeStateTab;
...
//                           ]}
//                           onPress={() => setActiveStateTab(code)}
//                         >
//                           <MapPin
//                             size={13}
//                             color={
...
//                     You are currently editing rates for{" "}
//                     {getStateLabel(activeStateTab)}
//                   </Text>
//                 </View>
