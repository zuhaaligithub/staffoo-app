...
export default function DocumentsScreen({ navigation }: Props) {
  ...
  const handleOpenEditModal = (item: Document) => {
    ...
    if (isVerifiableDocType(docType)) {
      const bridging = isVisaDocType(docType) && isBridgingVisaStaff();
      if (bridging) {
        // Expiry isn't required for bridging-visa users; a document
        // number plus either an expiry or an existing work rights file
        // is enough to consider it already verified.
        setIsVerified(
          !!(item.document_no && (item.document_expiry || item.working_rights)),
        );
      } else {
...
//   const isVerifyDisabled = verifying || !documentNumber.trim() || isVerified;
...
//               {/* Work Rights Document Section - Rendered only after verification or if bridging visa */}
//               {(showWorkDocument || isBridgingVisaDocument()) && (
//                 <View style={styles.workRightsSection}>
//                   <View style={styles.sectionHeader}>
//                     <FileText
...
//   },
//   verifyButtonText: { color: THEME.teal, fontWeight: "bold", fontSize: 14 },
//   verifyButtonTextDisabled: { color: THEME.textMuted },
//   dateButton: {
//     flexDirection: "row",
//     justifyContent: "space-between",
//     alignItems: "center",
