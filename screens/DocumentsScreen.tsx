...
//   // Consider the contractor ready for rates when either the server-side
//   // `profile_completion_percentage` is 100, OR when all documents shown
//   // on this screen have an uploaded file. This covers cases where the
//   // backend hasn't yet updated the profile percentage but the uploads
//   // are present locally.
//   const haveAllDisplayedDocs =
//     Array.isArray(displayedDocuments) &&
...
//     displayedDocuments.every(
//       (d) => !!(d.file && String(d.file).trim().length > 0),
//     );

//   // Require local document completeness only: the Proceed button will be
//   // enabled only when every displayed state document has a file. We no
//   // longer rely on the server-side `profile_completion_percentage`.
//   const isProfileComplete = haveAllDisplayedDocs;
