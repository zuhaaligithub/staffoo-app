...
const StaffFormsScreen = ({ navigation }: any) => {
  ...
  const [tfnAddressSuggestions, setTfnAddressSuggestions] = useState<
    PlaceSuggestion[]
  >([]);
  const [showTfnSuggestions, setShowTfnSuggestions] = useState(false);
  const [onboardAddressSuggestions, setOnboardAddressSuggestions] = useState<
    PlaceSuggestion[]
  >([]);
  ...
  // `profileActivated` becomes true exactly once: when the user has
  // completed all three forms AND tapped "Finish" on the completion alert.
  // It is persisted to AsyncStorage under a per-user key so it survives
  // logout/login and revisiting this screen, and it permanently suppresses
  // the completion alert from then on — including after later edits to any
  // of the three forms.
  ...
  const handleTfnAddressChange = (text: string) => {
    setTfnAddress(text);
    ...
    addressDebounceRef.current = setTimeout(() => {
      fetchAddressSuggestions(
        ...
        setShowTfnSuggestions,
      );
    }, 400);
  };

  const handleOnboardAddressChange = (text: string) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setUserId(parsed.id);
        // Check activation status as early as possible so no stale
        // "not activated" render can ever trigger the completion alert.
        await syncActivationStatus(parsed.id);
  ...
  // Resolves whether this user's profile has already been activated.
  // Source of truth #1: local persisted flag (works offline, survives a
  // normal logout that only clears the auth token / user object, and
  // survives revisiting this screen or switching tabs).
  // Source of truth #2 (optional/best-effort): a server-provided flag on
  // the staff record, in case the backend later starts returning one. If
  // found, it's mirrored into AsyncStorage so subsequent checks are local.
  // This function only ever moves profileActivated from false → true; it
  // never resets an already-activated profile back to inactive.
  const syncActivationStatus = async (
    id: number | string,
    serverData?: Record<string, any>,
  ): Promise<boolean> => {
    try {
      const stored = await AsyncStorage.getItem(`profile_activated_${id}`);
      if (stored === "true") {
      ...
      const serverActivated = !!(
        serverData?.profile_activated ||
        serverData?.is_profile_active ||
        serverData?.profile_status === "active" ||
        serverData?.staff?.profile_activated
      );

      if (serverActivated) {
        await AsyncStorage.setItem(`profile_activated_${id}`, "true");
        setProfileActivated(true);
        return true;
      }
      ...
      console.log("syncActivationStatus error:", e);
      return false;
    }
  };

  // Called only when the user taps "Finish" on the one-time completion
  // alert. Persists activation locally (and best-effort on the server if
  // an endpoint exists), then navigates to the Profile page.
  const handleActivateProfile = async () => {
    ...
    try {
      ...
      try {
        await axios.post(
          `${BASE_URL}/api/activate-profile`,
          { user_id: userId },
          { headers: { Authorization: `Bearer ${token}` } },
        );
      } catch (apiErr) {
        // Best-effort only: even if this endpoint doesn't exist yet or
        // fails, we still persist locally so the alert never reappears.
        console.log("activate-profile API error:", apiErr);
      }

      await AsyncStorage.setItem(`profile_activated_${userId}`, "true");
      setProfileActivated(true);
      ...
      console.log("Profile activation error:", err);
      Toast.show({
        type: "error",
  ...
  // Capture a fresh baseline for whichever tab just finished loading from
  // the server. tabLoading only toggles during the load flow, so this never
  // fires while the user is simply typing.
  useEffect(() => {
  ...
  // If a tab is marked "saved" but its fields have drifted from the last
  // saved baseline, flip it back to unsaved so the Save button reappears.
  useEffect(() => {
    if (tabLoading || tfnSnapshotRef.current === null) return;
    if (tfnSaved && buildTfnSnapshot() !== tfnSnapshotRef.current) {
      setTfnSaved(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
  ...
  useEffect(() => {
    if (tabLoading || superSnapshotRef.current === null) return;
    if (superSaved && buildSuperSnapshot() !== superSnapshotRef.current) {
      setSuperSaved(false);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
  ...
  useEffect(() => {
    if (tabLoading || onboardSnapshotRef.current === null) return;
    if (
      onboardingSaved &&
      buildOnboardSnapshot() !== onboardSnapshotRef.current
    ) {
  ...
  const getStaffInfo = async (staffId: number | string) => {
    try {
      const token = await getToken();

      const response = await axios.get(
        `${BASE_URL}/api/get-staff-info/${staffId}`,
        {
          headers: {
      ...
      // Keep the activation flag in sync in case the backend exposes one
      // (never flips an already-activated profile back to inactive).
      await syncActivationStatus(staffId, data);
      ...
      if (data.passport_expiry) {
        setPassportExpiry(formatDateToDDMMYYYY(data.passport_expiry));
        setPassportExpiryBackend(data.passport_expiry);
      }
      if (data.passport_attachment) {
        setPassportDoc(data.passport_attachment);
      ...
      if (data.security_license_expiry) {
        setSecurityExpiry(formatDateToDDMMYYYY(data.security_license_expiry));
        setSecurityExpiryBackend(data.security_license_expiry);
        // Lock the expiry since it already exists from server
        if (data.security_license_no) {
          setIsSecurityVerified(true);
        }
      }
      if (data.security_license_file) {
      ...
      if (data.first_aid_expiry) {
        setFirstAidExpiry(formatDateToDDMMYYYY(data.first_aid_expiry));
        setFirstAidExpiryBackend(data.first_aid_expiry);
      }
      if (data.first_aid_file) {
...
<div class="page">
  ...
  const downloadFileToDevice = async (
    ...
    if (Platform.OS === "android") {
      try {
        // Normalize to a plain path (no "file://") for react-native-blob-util.
        ...
        console.error("Download to device error (MediaStore):", err);
        throw err;
      }
    }
  ...
  const generateUploadAndDownloadPdf = async (
    ...
    try {
      ...
      try {
        ...
        // The form itself was saved & uploaded successfully — only the
        // local "save a copy to Downloads" step failed. Don't block the
        // save flow or show a scary error for this secondary step.
        console.error("Local download step failed:", downloadErr);
        Toast.show({
          type: "success",
          text1: "✓ Form saved",
          text2: "Couldn't save a local copy, but your data is safe.",
        });
      }
    } catch (error: any) {
  ...
  const fetchFormData = async (id: number | string) => {
    try {
      ...
      if (!formData) {
        ...
        if (activeStaffTab === "tfn") setTfnSaved(false);
        else if (activeStaffTab === "super") setSuperSaved(false);
        else if (activeStaffTab === "onboarding") setOnboardingSaved(false);
        return;
      }
      ...
      if (activeStaffTab === "tfn") {
        ...
        setTfnSaved(!!formData.signature);
      } else if (activeStaffTab === "super") {
        ...
        setSuperSaved(!!formData.signature);
      } else if (activeStaffTab === "onboarding") {
        ...
        if (formData.full_name) setOnboardFullName(formData.full_name);
        setOnboardMobile(formData.mobile || "");
        setOnboardEmail(formData.email || "");
        if (formData.address) setOnboardAddress(formData.address);
        ...
        setOnboardingSaved(!!formData.signature);
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (savedTab === "onboarding") {
        ...
        // Superannuation is the final step. At this point onboarding and
        // TFN were already required (and thus already saved) to unlock
        // this tab, and this save just succeeded for super itself — so
        // all three forms are now complete. Show the one-time completion
        // alert, unless the profile has already been activated before
        // (e.g. the user came back and re-saved this tab after editing).
        if (!profileActivated) {
          showCompletionAlert();
        }
      }
  ...
  return (
    <SafeAreaView style={s.container}>
        ...
        {[
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => goToTab(tab.key as StaffTab)}
              activeOpacity={0.7}
            >
      ...
      >
        ...
        {activeStaffTab === "super" && (
          <View style={s.card}>
            ...
            {/* Full Name — editable, autofilled from staff info */}
            <Field label="Full Name">
              <StyledInput
                value={superFullName}
              ...
              <View style={{ flex: 1 }}>
                ...
                <Field label="Email Address">
                  <View style={[s.input, s.disabledInput]}>
                    <Text style={s.disabledText} numberOfLines={1}>
                      {onboardEmail || "name@domain.com"}
                    </Text>
                  </View>
                </Field>
              </View>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#0B1220" }}>
          ...
          <View style={{ flex: 1, marginHorizontal: 16, marginTop: 12 }}>
            ...
            * { box-sizing: border-box; }
            ...
            /* Hide default footer completely */
          ...
          <View style={s.sigActionCard}>
            ...
            >
              ...
              <Text style={s.sigBtnClearText}>Clear</Text>
            </TouchableOpacity>
            ...
            >
              ...
              <Text style={s.sigBtnUndoText}>Undo</Text>
            </TouchableOpacity>
            ...
            >
              <Check size={18} color="#03211E" />
              <Text style={s.sigBtnSaveText}>Save Signature</Text>
            </TouchableOpacity>
          </View>
        </SafeAreaView>
      </Modal>
      ...
      <Modal
        ...
        onRequestClose={() => {}}
      >
        <View style={s.modalOverlay1}>
          <View style={s.alertContainer}>
            <View style={s.alertIconContainer}>
              <BadgeCheck size={36} color={BRAND} />
            </View>
            <Text style={s.alertTitle}>Forms Complete</Text>
            <Text style={s.alertMessage}>
              Great! All required documents and forms are complete. Click Finish
              & Activate to activate your profile.
            </Text>
            <TouchableOpacity
              style={s.alertButton}
              onPress={() => {
              ...
              activeOpacity={0.8}
            >
              <Text style={s.alertButtonText}>Finish & Activate</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
...
// Replace your current IdCheckRow component with this improved version:
const IdCheckRow = ({ label, checked, onPress }: any) => (
  <TouchableOpacity style={s.idRow} onPress={onPress} activeOpacity={0.7}>
    <Text style={s.idRowText} numberOfLines={2} ellipsizeMode="tail">
      {label}
    </Text>

    <View style={[s.idCheckCircle, checked && s.idCheckCircleOn]}>
      {checked && <Check size={14} color={BRAND_DARK} />}
    </View>
  </TouchableOpacity>
);
...
const s = StyleSheet.create({
  ...
  label: { fontSize: 10, fontWeight: "600", color: "#929294", marginBottom: 4 },
  ...
  suggestionText: { color: "#fff", fontSize: 13, lineHeight: 20 },
  ...
  dateBtnText: { color: "#fff", fontSize: 12 },
  ...
  docPickBtnText: { color: "#fff", fontSize: 11, fontWeight: "700" },
  ...
  secVerifyBtnText: { color: "#fff", fontWeight: "700", fontSize: 10 },
  secVerifiedBadge: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: BRAND,
  ...
  sigHint: {
    backgroundColor: "rgba(46, 177, 226, 0.08)",
    paddingHorizontal: 20,
    paddingVertical: 10,
    borderBottomWidth: 1,
    borderBottomColor: "rgba(46, 177, 226, 0.15)",
  },
  sigHintText: {
    fontSize: 13,
    color: "#94A3B8",
  ...
  sigCanvasCard: {
    ...
    backgroundColor: "#fff",
    borderWidth: 1.5,
    borderColor: "rgba(46, 177, 226, 0.25)",
    // soft shadow
    shadowColor: "#000",
