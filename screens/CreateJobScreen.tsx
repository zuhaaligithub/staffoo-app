...
//           // 1. Remove the original long shift
//           const targetDay = newList[dayIndex];
//           targetDay.shifts = targetDay.shifts.filter(
//             (_, i) => i !== shiftIndex,
//           );

//           // 2. Add split shifts to the CORRECT day (using the original dayIndex)
//           newShifts.forEach((shift) => {
//             // Use the original day's date as base to prevent date drift
//             const targetDate = new Date(newList[dayIndex].date);
//             targetDate.setHours(0, 0, 0, 0);

//             const shiftStart = new Date(shift.startTime);
//             shiftStart.setHours(0, 0, 0, 0);

//             // Only create new day if split actually crosses days
//             if (!isSameDay(targetDate, shiftStart)) {
//               let idx = newList.findIndex((d) => isSameDay(d.date, shiftStart));
//               if (idx === -1) {
//                 newList.push({ date: shiftStart, shifts: [shift] });
//               } else {
//                 newList[idx].shifts.push(shift);
//               }
//             } else {
//               // Most common case: keep on same day
//               targetDay.shifts.push(shift);
//             }
//           });

//           // Clean up empty days and sort
//           const cleaned = newList.filter((day) => day.shifts.length > 0);
//           return cleaned.sort((a, b) => a.date.getTime() - b.date.getTime());
//         });
//       }
//       setPickerTarget(null);
//       return;
...
export default function CreateJobScreen() {
  ...
  const onTimePickerChange = (_: any, selectedDate?: Date) => {
    if (Platform.OS === "android") {
      setPickerVisible(false);
      if (selectedDate) applyTimeChange(selectedDate);
