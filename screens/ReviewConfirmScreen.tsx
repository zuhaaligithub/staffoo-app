...
export default function ReviewConfirmScreen() {
  ...
  const getChargeAmount = (total: number) => {
    // `total` is expected to already be the GST-inclusive total AFTER any
    // discounts which are applied on the EX-GST subtotal. For a full
    // payment we charge the full GST-inclusive total; for split we charge
    // half of that amount now.
    return parseFloat(
  ...
  const submitJob = async (intentId: string | null) => {
    ...
    const payload: JobPostPayload = {
      ...
      ...(isStateNotMatched
        ...
        : {
            financials: {
              // `base_total_inc_gst` is the GST-inclusive total after the
              // discount has been applied on the ex-GST subtotal.
  ...
  // ── Pricing rule ──────────────────────────────────────────────────────
  // Both plans are calculated independently from the ex-GST subtotal so
  // that switching the selected plan never changes the OTHER plan's price.
  //
  // Pay In Full: discount (5%) is applied to the ex-GST subtotal FIRST,
  // then GST (10%) is calculated on the DISCOUNTED subtotal.
  ...
  // Split Payment: no discount applies. GST (10%) is calculated on the
  // FULL (non-discounted) ex-GST subtotal, then 50% of that is due now.
  ...
  // The GST-inclusive total for whichever plan is currently selected —
  // used for the actual charge/payload, not for displaying the other
  // plan's card.
  const totalIncGST =
            ...
            <View style={styles.rateCard}>
              ...
              {costBreakdown.breakdown.length === 0 ? (
                ...
                <>
                  ...
                  {/* Totals block — order follows the pricing rule:
                      Subtotal -> Discount (if any) -> GST (on discounted
