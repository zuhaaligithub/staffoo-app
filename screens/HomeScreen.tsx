...
const COLORS = {
  ...
  textMuted: "#4A6080",
  success: "#34C88A",
  successBg: "rgba(52,200,138,0.14)",
  successBorder: "rgba(52,200,138,0.3)",
  danger: "#F87171",
  dangerBg: "rgba(248,88,88,0.12)",
  dangerBorder: "rgba(248,88,88,0.3)",
  warning: "#F5A623",
  warningBg: "rgba(245,166,35,0.08)",
  warningBorder: "rgba(245,166,35,0.3)",
  blue: "#4B9EF5",
  blueBg: "rgba(75,158,245,0.14)",
  blueBorder: "rgba(75,158,245,0.3)",
  purple: "#A78BFA",
  heroBg1: "#0e2231",
  heroBg2: "#071318",
};
  ...
  useEffect(() => {
    const loadProfileAndLocation = async () => {
      try {
        ...
        // 2) Try local user object (works for staff)
        const localPath =
          parsedUser?.staff?.profile_image ||
          parsedUser?.customer?.profile_image ||
          parsedUser?.contractor?.profile_image ||
        ...
        if (localPath) {
          const uri =
            ...
            (localPath.startsWith("http://") ||
              ...
              ? localPath
              : `${BASE_IMAGE_URL}${localPath}`;
          setProfileImage(uri);
          return;
        }

        // 3) Fallback: fetch full profile from API (customer + contractor)
        if (parsedUser?.id) {
          try {
            ...
            const apiPath =
              profile?.staff?.profile_image ||
              profile?.customer?.profile_image ||
              profile?.contractor?.profile_image ||
            ...
            if (apiPath) {
              const uri =
                ...
                (apiPath.startsWith("http://") ||
                  ...
                  ? apiPath
                  : `${BASE_IMAGE_URL}${apiPath}`;

              setProfileImage(uri);
              // Cache so next open is instant
              await AsyncStorage.setItem("profileImage", uri);
            }
          } catch (e) {
            console.log("Profile image fetch error:", e);
          }
        }
      } catch (e) {
        console.log("loadProfileAndLocation error:", e);
      }
    };
            ...
            >
              {sites.length === 0 ? (
                ...
                sites.map((job: any) => {
                  ...
                  const endTime =
                    job.end?.split(" ")[1]?.slice(0, 5) || "--:--";
                  const statusStyle = getStatusStyle(statusInfo.key);
...
const styles = StyleSheet.create({
  ...
  badgeText: { color: COLORS.text, fontSize: 9.5, fontWeight: "700" },
  ...
  mainScroll: { flex: 1 },
  mainScrollContent: { paddingBottom: 100 },
  section: { paddingHorizontal: 15, marginTop: 18 },
  ...
  docsRowLabel: { fontSize: 13, fontWeight: "700", color: COLORS.text },
  docsRowText: { fontSize: 12, color: COLORS.textSecondary, marginTop: 2 },
  ...
  loadingText: { marginTop: 15, fontSize: 16, color: COLORS.text },
