...
//   //     console.log("Places API Response:", json.status);
...
//                 // Clear dependent fields when address becomes empty
//                 if (!text?.trim()) {
//                   clearAddressFields();
//                 }
...
//                 onPress={() => {
//                   setAddress("");
//                   clearAddressFields(); // ← Important fix
//                   setPredictions([]);
//                   setShowSuggestions(false);
...
//               icon={Building2}
//               label="State"
//               value={getFullStateName(stateValue)}
//               onChange={() => {}}
//               editable={false}
//               placeholder="Auto-filled from address"
//             />
...
export default function ProfileSetupScreen({ navigation }: Props) {
    ...
    const initializeProfile = async () => {
      try {
        ...
        if (profile?.user_type === "contractor") {
          ...
          setCompanyName(profile?.contractor?.company_name ?? "");
          setRegistrationNumber(profile?.contractor?.registration_number ?? "");
          // Run through the formatters so whatever the backend returns
          // (raw digits or already dash-formatted) always displays
          // consistently as "123-456-789" / "12-345-678-901".
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      // CONTRACTOR FIELDS
      // acn / abn are sent exactly as displayed (dash-formatted, e.g.
      // "123-456-789" / "34-535-555-353") to match what the backend has
      // been storing. They're always included — even as "" — now that
      // authApi's updateUserProfile appends whenever the key is present
      // (`!== undefined`) instead of only when truthy, so a value you
      // type actually reaches the server and a cleared field actually
      // clears server-side too.user-update
      if (userType === "contractor") {
        payload.company_name = companyName.trim();
        payload.registration_number = registrationNumber.trim();
