  ...
  useEffect(() => {
    const initializeProfile = async () => {
      try {
        ...
        // ✅ FIXED: set image AFTER profile is available
        const BASE_IMAGE_URL = 'https://apis.staffoo.com.au/storage/';

        if (profile?.staff?.profile_image) {
          const fullImageUrl = `${BASE_IMAGE_URL}${profile.staff.profile_image}`;
         ...
         if (profile?.customer?.profile_image) {
          const fullImageUrl = `${BASE_IMAGE_URL}${profile.customer.profile_image}`;
          ...
          setProfileImage(fullImageUrl);
            ...
            if (profile?.contractor?.profile_image) {
          const fullImageUrl = `${BASE_IMAGE_URL}${profile.contractor.profile_image}`;
          ...
          <View style={styles.field}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                dropdownIconColor="#666"
              >
                <Picker.Item label="Select Residential Status" value={null} />
                <Picker.Item label="Student Visa" value="student_visa" />
                <Picker.Item label="Bridging Visa" value="bridging_visa" />
                <Picker.Item label="Citizen" value="citizen" />
                <Picker.Item label="Permanent Residence" value="permanent_residence" />
                <Picker.Item label="Visa Subclass 485" value="visa_485" />
