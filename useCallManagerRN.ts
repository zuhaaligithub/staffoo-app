...
// //         console.warn(
// //           `[EchoRN] Transient error (${
// //             type || code
// //           }). Waiting for Pusher auto-reconnect...`,
// //         );
// //         return;
// //       }
...
// //     console.log(
// //       `[EchoRN] Subscribed to notifications.${userId} and user.${userId}`,
// //     );
// //     return true;
// //   } catch (err: any) {
...
// // useCallManagerRN.ts (Partial update to the echoSubscribe function)
// async function echoSubscribe(): Promise<boolean> {
//   try {
//     const token = await getAuthToken();
//     const userRaw =
...
//     if (_subscribedUserId === userId && _echo) {
//       const state = _echo.connector?.pusher?.connection?.state;
//       console.log('[EchoRN] ℹ️ Existing subscription check', { userId, state });
//       if (state === 'connected' || state === 'connecting') return true;
//     }

//     echoUnsubscribe();
//     _echo = await getEchoInstance();
//     console.log('[EchoRN] ℹ️ Echo instance created', {
//       hasConnector: Boolean(_echo?.connector),
//       hasPusher: Boolean(_echo?.connector?.pusher),
//       connectionState: _echo?.connector?.pusher?.connection?.state,
//     });
//     // CRITICAL FIX: Ensure the internal pusher connector is ready before calling .private()
//     if (!_echo || !_echo.connector || !_echo.connector.pusher) {
//       console.warn('[EchoRN] ⏳ Echo connector not ready yet.');
//       return false;
//     }
...
//       .listen(EVENT, (data: any) => handleEchoEvent(data, userId!))
//       .error((err: any) => {
//         if (err?.status === 401) _subscribedUserId = null;
//       });
...
//     console.log(`[EchoRN] ✅ Subscribed to userId: ${userId}`);
//     return true;
//   } catch (err: any) {
//     console.error('[EchoRN] ❌ Subscribe failed:', err);
//     return false;
...
//         Toast.show({
//           type: 'error',
//           text1: acceptRes?.error || 'Failed to accept',
//         });
//         return null;
//       }
...
//       console.log(
//         `[CallManager] Fetching token for channel: ${channelName}, uid: ${uid}`,
//       );
...
//       console.log(
//         `[CallManager] endCall logic (reject: ${isReject}, id: ${activeCallId})`,
//       );
...
//       } else {
//         console.warn(
//           '[CallManager] ⚠️ No activeCallId found to end call on server',
//         );
//       }
//     },
//     [],
...
async function echoSubscribe(): Promise<boolean> {
  try {
    ...
    await waitForConnection(connection, 12000);
